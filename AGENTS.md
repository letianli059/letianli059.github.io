...
This repository powers Letian Li's public website at `https://letianli059.github.io/`. The canonical personal content is in `_pages/about.md`, `_pages/publications.md`, `_pages/talks.md`, `_pages/awards.md`, `_pages/cv.md`, `_bibliography/papers.bib`, `_data/cv.yml`, `_data/socials.yml`, and `assets/img/prof_pic.jpg`.

- Blog, projects, repositories, teaching, people, books, news, and submenu pages are intentionally absent. Do not restore template examples in place of missing personal content.
- Preserve verified publication status. Use `published`, `accepted`, or `manuscript`; do not invent dates, venues, abstracts, DOI links, or files.
- `assets/js/theme.js` intentionally shadows `al_folio_core` 1.0.15 so a new visitor starts in dark mode. Re-audit this one-line behavioral override when upgrading the core gem.
- Do not commit, merge, push, publish, or change GitHub Pages settings unless the user explicitly requests it.
- The previous production setup deploys `master` directly. The al-folio deployment workflow instead generates `gh-pages`; coordinate the Pages source change before the first redesign push.
...
3. **This user site is published at the domain root.** `_config.yml` deliberately leaves `baseurl` blank. Do not restore the upstream `/al-folio` baseurl; local preview is served from `/`.
